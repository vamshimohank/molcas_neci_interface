...
data_col: index (starting from 0) of the column containing the data.'''
    ...
    def reblock(self):
        ...
        block_size = int(len(self.data)/2)
...
where N is the number of elements in the data set and hence we obtain:
    ...
    def show_blocking(self, plotfile=None):
        """
        Print out the blocking data and show a graph of
        the behaviour of the standard error with block size.
        If plotfile is given, then the graph is saved to the specifed
        file rather than being shown on screen.
        """
        ...
        for s in range(len(self.data[0].stats)):
            ...
            for cov in iter(self.covariance.values()):
                print('%+-#14.5e' % (cov[s]), end=' ')
            for comb in iter(self.combination_stats.values()):
        ...
        if PYLAB:
            ...
            for (i, data) in enumerate(self.data):
                ...
                pylab.errorbar(blocks, se, yerr=se_error, ecolor='g')
