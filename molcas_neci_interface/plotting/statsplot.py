...
class colour_manager:
    def __init__(self):
        ...
        self.colours = ['r', 'g', 'b', 'c']
        # from cycler import cycler
        # self.colors = cycler(color='bgrcmyk')
        self.ind = 0
        self.ref_point = 0
        self.frozen = False
        self.ind_max = 0
...
def read_cols(f, last_iter=None, last_im_time=None):
    ...
    for line in f:
        ...
        if line.decode().split()[0] == '#':
            # print('Entered')

            # The first line contains the column header. Split this up
            # into sections
            if not found_hdr and re_label.match(line.decode()):
                found_hdr = True
                col_hdrs = [s.strip() for s in re_label_split.split(line.decode())
                            if s != '']
            ...
            # print(line.decode().split())
            bits = [float(s) for s in line.decode().split()]
...
def output_file(fin):
    ...
    out_base_in = ['OUT', 'OUTPUT', 'OUT.FCIMC', 'OUTPUT.FCIMC', 'out', 'output.FCIMC', 'output', 'out_', 'neci.out',
                   'neci.out_']
    out_base = []
...
class plotter():
    ...
    def plot_data(self, f,fout=None,what='walkers'):
        """
        Actually plot the data

        :param f: file pointer to FCIQMCStats
        :return:
        """
        ...
        if self.verbose:
            print('Plotting FCIMCStats with output file neci.out')
        ...
        # tau_changes = None
        # if fout is not None and self.E.total_energies:
        #     ref_E, E_final, tau_changes = process_output(fout)
...
def process_output(fout):
    ...
    for line in fout:
        if it is not None:
            ...
            m = re_start_fciqmc.match(line.decode())
            if m:
                HF_energies = []
                tau_changes = []
                E_final = None
            ...
            if re_startiter.match(line.decode()):
                it = 0
                continue

            # Have we hit the initial setting of the HF energy?
            m = re_HF_D0.match(line.decode())
            if m:
            ...
            # Change of tau?
            m = re_init_tau.match(line.decode())
            if m:
